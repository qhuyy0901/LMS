...
public class NguoiDungController(IDichVuNguoiDung dichVu) : ControllerBase
{
    ...
    public async Task<IResult> XoaTaiKhoan()
    {
        var userId = TroGiup.LayUserId(User);
        if (userId is null) return Results.Unauthorized();
        return await dichVu.XoaTaiKhoanAsync(userId) ? Results.Ok(new { message = "Tài khoản đã được xóa vĩnh viễn." }) : Results.NotFound(new { message = "Không tìm thấy người dùng" });
    }
    ...
    public async Task<IResult> DatAvatar()
    {
        ...
        return url is null ? Results.NotFound(new { message = "Không tìm thấy người dùng" }) : Results.Ok(new { message = "Tải ảnh thành công", avatarUrl = url });
    }
    ...
    public async Task<IResult> XoaAvatar()
    {
        var userId = TroGiup.LayUserId(User);
        if (userId is null) return Results.Unauthorized();
        return await dichVu.XoaAvatarAsync(userId) ? Results.Ok(new { message = "Đã xóa ảnh đại diện", avatarUrl = (string?)null }) : Results.NotFound(new { message = "Không tìm thấy người dùng" });
    }
    ...
    public async Task<IResult> DanhDauDaDoc(string id)
    {
        var userId = TroGiup.LayUserId(User);
        if (userId is null) return Results.Unauthorized();
        return await dichVu.DanhDauDaDocAsync(userId, id) ? Results.Ok(new { message = "Đã đánh dấu đã đọc" }) : Results.NotFound(new { message = "Không tìm thấy thông báo" });
    }
}
